...
#' @references  ...
#' @export
threshold_cluster_nmm <- function(x, thresholds=100, G=2, ...) {
  require(mclust)
  ...
  nmm <- densityMclust(x, G=G)
  
  
  rng <- nmm$range
  ys <- min_f1f2(thresholds, nmm)
  xs <- c(thresholds, thresholds[1])
  ys <- c(ys, ys[1])
  pred <- predict(nmm, thresholds, what="cdens")
  pred.scl <- pred*NA
  for (i in 1:nmm$G)
  ...
  idx_th <- which(thresholds == threshold)
